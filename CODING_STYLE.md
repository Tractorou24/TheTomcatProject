# TheTomcatProject coding style

**Important: Make sure you use the visual studio formatter from VS2019 or later!**

This document describes the coding style used for C/C++ code in TheTomcatProject. All code should conform to this style.
...
When a function raises an exception, prefix with `try_`. As it tries, you can surround it with a try/catch to catch any error from that function.

###### Right:

```cpp
unsigned int try_altitude();
```
...
`using` statements are NOT acceptable. It can only be considered for the test framework.
...
Do not omit "int" when using "unsigned" modifier. Do not use "signed" modifier. Use "int" by itself instead.
...
Use style where `const` is written on the left side of the type being qualified.
...
Use `const`, `constexpr`, `constval` anywhere you can.
